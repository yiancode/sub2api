...
func TestApplyAccountStatsCost_AppliesDeepSeekPeakFromPricingAt(t *testing.T) {
	channel := &Channel{
		ID:                         1,
		Status:                     StatusActive,
		ApplyPricingToAccountStats: false,
	}
	cs := newTestChannelServiceForStats(t, channel, 10, "deepseek")
	bs := newTestBillingService()
	tokens := UsageTokens{InputTokens: 1000, OutputTokens: 500, CacheReadTokens: 1000}
	offPeakTotal := 1000*2.2e-7 + 500*6.6e-7 + 1000*7e-9
	usageLog := &UsageLog{}
	...
	applyAccountStatsCost(
		context.Background(), usageLog, cs, bs,
		1, 10, "deepseek-v4-flash", "deepseek-v4-flash",
		tokens, 999,
		time.Date(2026, 8, 24, 2, 0, 0, 0, time.UTC),
	)

	require.NotNil(t, usageLog.AccountStatsCost)
	require.InDelta(t, offPeakTotal*2, *usageLog.AccountStatsCost, 1e-10)

	offPeakLog := &UsageLog{}
	applyAccountStatsCost(
		context.Background(), offPeakLog, cs, bs,
		1, 10, "deepseek-v4-flash", "deepseek-v4-flash",
		tokens, 999,
		time.Date(2026, 8, 24, 12, 0, 0, 0, time.UTC),
	)
	require.NotNil(t, offPeakLog.AccountStatsCost)
	require.InDelta(t, offPeakTotal, *offPeakLog.AccountStatsCost, 1e-10)
}
