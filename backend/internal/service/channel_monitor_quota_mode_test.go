...
func TestMonitorAccountQuotaCapability_Matrix(t *testing.T) {
	cases := []struct {
		...
		{
			// 自定义/中转域名 kimi coding：按 Platform 识别，额度探测走账号 base_url。
		...
		{
			name: "custom-domain zhipu coding ok",
			account: &Account{ID: 16, Platform: domain.PlatformZhipu, Type: AccountTypeAPIKey,
